...
class EyeDataGrid
{
	...
	private $filterex = false; // Current filterex
	...
	*/
	...
	$filterex = (isset($_GET['indicehiden'])) ? $_GET['indicehiden'] : ''; // indicehiden clause
		...
		//### I.- aqui ir viendo como integrar un filtro, multiple
		// Set the filter
		if ($filter)
		{
	...
	if ($filterex)
		{
		  $filterexplode=explode("__",$filterex);
		  for ($iij=0;$iij<count($filterexplode);$iij++)
		    {
			list($columnex[$iij], $valueex[$iij]) = $this->parseInputCond($filterexplode[$iij]);
		    }
			$this->setFilterex($columnex, $valueex);
	...
	*/
	...
	{
	  //### II.-  aqui crear una nueva variable para albergar el cumulo de columnas
		$this->filter = array('Column' => $column,
							'Value' => $value);
	}
	private function setFilterex($columnex, $valueex)
	{
	  //### II.-  aqui crear una nueva variable para albergar el cumulo de columnas
		$this->filterex = array('Columnex' => $columnex,
							'Valueex' => $valueex);
	}
	...
	{ 
		...
		switch ($type)
		{
			...
			case self::STDCTRL_DELETE:
				$this->controls[] = '<a ' . $action . '><img src="' . $this->image_path . $this->img_delete . '" alt="Delete" title="Delete" class="tbl-control-image" width="30" height="30"></a>';
				break;
			default:
	...
	private function buildHeader()
	{
		...
		foreach ($headers as $t)
		{
			...
			if ($this->hide_order)
				...
				//###4 aqui poner campos hidden

				echo '<td class="tbl-header"><a href="javascript:;" onclick="tblSetOrder(\'' . $t . '\', \'' . $order . '\')">' . $header . "</a>";

				echo '<input type=\'hidden\' name=\'a'.$indicehidden.'\' id=\'a'.$indicehidden.'\' value=\''.$t.'\'/>';
				$indicehidden++;
			...
			// Add filter if allowed and only if the column type is not "special"
			...
			{
			  //### III.- en el if reconocer todas las columnas con datos en el filtro extendido
			  $llave=array_search($t,$this->filterex['Columnex']);
			  if (
			      ($this->filter['Column'] == $t and !empty($this->filter['Value']))
			      ||
			      (in_array($t,$this->filterex['Columnex']) and !empty($this->filterex['Valueex'][$llave])) 
			      )
				{
				  //echo $t."aa";
                                  //echo $llave."llav";
				  //echo $filterex;
                                  //print_r($this->filterex['Valueex']);
				  ...
				  //$filter_display = 'none';
					...
					if (!is_null($llave) )
					  {
					    //echo "si";
					  $filter_value=$this->filterex['Valueex'][$llave];
			  ...
			  //1.- abajo incluir if que en caso de ser fecha ubicará campos con respectivo javascript para fecha
			//  echo "<br>el t ".$t;
			  
			  if(substr_compare($t, "fecha", 0, 5)!=0)
			    {

			      echo '<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilterFelipe(\'' . $t . '\')">filtrar</a></div>';
			    } 
			  else
			    {
			      $filter_valuef=explode("!ff",$filter_value);
			      // <input type="text" readonly="" name="fecha2" id="searchGadget3" onclick="scwShow(this,event);">
  echo '<a href="javascript:;" onclick="tblShowHideFilter(\'' . $t . '\')"><img src="' . $this->image_path . 'filter.gif" class="tbl-filter-image"></a><br><div class="tbl-filter-box" id="filter-' . $t . '" style="display:' . $filter_display . '"><input type="text" size="6" id="filter-valuef1-' . $t . '" value="'.$filter_valuef[0].'" onclick="scwShow(this,event);" readonly="" >&nbsp;<input type="text" size="6" id="filter-valuef2-' . $t . '" value="'.$filter_valuef[1].'" onclick="scwShow(this,event);" readonly="">&nbsp;<input type="hidden" id="filter-value-' . $t . '" value="'.$filter_value.'">&nbsp;<a href="javascript:;" onclick="tblSetFilterFelipe(\'' . $t . '\')">filtrar</a></div>';
			    }
			}
			echo '</td>';
		}
	...
	*/
	public function printTable()
	{
		...
		//### aqui se formula el where
		if ($this->allow_filters and $this->filter)
		{
		  if (count($this->filterex['Columnex'])==1) // si es solo un criterio , lo que sigue
		    {
		  ...
		  else // si es mas de un criterio
		    {
		      for ($iij=0;$iij<count($this->filterex['Columnex'])-1;$iij++)
			{
			  //  if (!strstr($this->filterex['Valueex'][$iij], '%'))
			  //  $filterex_value[$iij] = '%' . $this->filterex['Valueex'][$iij] . '%';
			  // else

			    $filterex_value[$iij] = $this->filterex['Valueex'][$iij];
			    $filterex_value[$iij] =str_replace("*","%",$filterex_value[$iij]);
			    $filterex_value[$iij] =str_replace("!$!","\%",$filterex_value[$iij]);
			}
		    }
		  ...
		  if (count($this->filterex['Columnex'])==1) // si es solo un criterio , lo que sigue
			...
			//### III.- aqui ver como integrar filter modificado y pasar al where
		      ...
		      for ($iij=0;$iij<count($this->filterex['Columnex'])-1;$iij++)
			{
			  if ($this->select_where)
			    $filter_query = $filter_query . " AND ";
			  //### III.- aqui ver como integrar filter modificado y pasar al where
			  //3.- aqui integrar fecha para que cuando sea una columna tipo fecha, no se emplee el like
			  if(substr_compare($this->filterex['Columnex'][$iij], "fecha", 0, 5)==0)
			    {
			      if ($filterex_value[$iij]!='')
				{
				  $filter_valuefx=explode("!ff",$filterex_value[$iij]);
				  $filter_query .= "( `".$this->filterex['Columnex'][$iij] . "` >= '" .  $filter_valuefx[0] . "'  AND `".$this->filterex['Columnex'][$iij] . "` <= '" . $filter_valuefx[1] . "')";
				}
			    } 
			  else
			    {
			      $filter_query .= "(`" . $this->filterex['Columnex'][$iij] . "` LIKE '" . $filterex_value[$iij] . "')";
			    }
			}
		    }
		}
		...
		//echo "el query ".$query . ' ' . $order . ' ' . $limit;
	...
	foreach ($headers as $t)
		{
			...
			//###aca 1 es para calcular el parametro a enviar a printjavascript
	$this->printJavascript($columnas);
...
echo '<script type="text/javascript" src="calendario2.js" ></script>';
		...
		if ($this->row_count == 0)
			...
			while ($row = $this->_db->fetchAssoc($this->result))
			{
				...
				// Handle row selects
				if ($this->row_select) {
				  $lavar= $this->parseVariables($row, $this->row_select);					$querymas=str_replace(" ","+",$query);
                                  $querymas=str_replace("'","\'",$querymas);
				  $querymas=str_replace('"','\"',$querymas);
				  $querymas=str_replace("%","%25",$querymas);
				  $querymas=str_replace("*","Id",$querymas);
				  $lavarmas=str_replace("','Edita","&sql=".$querymas."','Edita",$lavar);
echo ' tbl-row-highlight" onclick="' . $lavarmas;
	...
	{
		self::printJavascript($this->column_count);
	...
	*
	*/
	public function printJavascript($iii)
	{
		if ($this)
		{
			$page = $this->page;
...
echo "function substr_compare (main_str, str, offset, length, case_insensitivity) {
    ...
    return ((main_str == str) ? 0 : ((main_str > str) ? 1 : -1));
		...
		echo "function tblSetPage(page) { 
textin='';otro='';paramcillo=''; 
for(ij=0;ij<".$iii.";ij=ij+1){
textin=document.getElementById('a' + ij).value;

otro=document.getElementById('filter-value-' + textin).value;
//if(substr_compare(textin, \"fecha\", 0, 5)==0)
elstr = textin.substr(0,5);
if (elstr=='fecha')
			    {
otro1=document.getElementById('filter-valuef1-' + textin).value;
otro2=document.getElementById('filter-valuef2-' + textin).value;
...
if (otro!=''){
paramcillo=paramcillo+textin+':'+otro+'__';
}
}
tblpage = page; params = '&page=' + page + '&order=' + tblorder +'&indicehiden='+paramcillo+ '&filter=' + tblfilter; updateTable(); }\n";
		...
		// ###1 Abajo integrar columnas con valor (poniendo hidden en otro lado )
		//	$iii=$this->column_count;
                // a continuacion poner lo del for que recorre de a0 a an pero en una instancia ej: solo para a0
		//2.-(caso fecha) abajo poner if para recojer los valores de las fechas en 2 filtros y concatena en  1.
		echo "function tblSetFilterFelipe(column) {textin='';otro='';paramcillo=''; 
for(ij=0;ij<".$iii.";ij=ij+1){
textin=document.getElementById('a' + ij).value;
...
otro=otro1+'!ff'+otro2;
     ...
     {
document.getElementById('filter-value-' + textin).value=0;
      otro='';
     }  
 }
...
val = document.getElementById('filter-value-' + column).value; tblfilter = column + ':' + val; tblpage = 1; params = '&indicehiden='+paramcillo+'&page=1&order=' + tblorder + '&filter=' + tblfilter; updateTable(); }\n";
...
echo "function tblShowHideFilterFelipe(column) { var o = document.getElementById('filter-' + column); if (o.style.display == 'block') { tblClearFilter(); } else {	o.style.display = 'block'; }}\n";
		echo "function tblReset() { params = '&page=1'; updateTable(); }\n";
		echo "</script>\n";
	}
}