...
//$wheresust2="AND ( freactopic.document_field_id=119 AND freactopic.value='Environment/Health' AND b.`id` = freactopic.`document_id` )";
...
{
$where1.="AND ( faction.document_field_id=127 AND  (faction.value='".$action."' or faction.value='NEW COMPANY') AND b.`id` = faction.`document_id` )";
} else if ($action=="Acquisition-Company")
{
$where1.="AND ( faction.document_field_id=127 AND  (faction.value='".$action."' or faction.value='ACQUISITIONS') AND b.`id` = faction.`document_id` )";
} else if ($action=="Expansion")
{
$where1.="AND ( faction.document_field_id=127 AND  (faction.value='".$action."' or faction.value='EXPANSION') AND b.`id` = faction.`document_id` )";
} else if ($action=="Mergers")
{
$where1.="AND ( faction.document_field_id=127 AND  (faction.value='".$action."' or faction.value='MERGERS') AND b.`id` = faction.`document_id` )";
} else if ($action=="Upgrades")
{
$where1.="AND ( faction.document_field_id=127 AND  (faction.value='".$action."' or faction.value='UPGRADES') AND b.`id` = faction.`document_id` )";
} else if ($action=="JV")
{
$where1.="AND ( faction.document_field_id=127 AND faction.value='".$action."' AND b.`id` = faction.`document_id` )";
}
else if ($action=="CORPORATE STRUCTURE")
{
$where1.="AND ( faction.document_field_id=127 AND faction.value='".$action."' AND b.`id` = faction.`document_id` )";
}
 else if ($action=="Alternative Fuels Project")
{
$where1.="AND ( faction.document_field_id=125 AND  (faction.value='".$action."' or faction.value='ALTERNATIVE FUELS PROJECT') AND b.`id` = faction.`document_id` )";
} else if ($action=="New Material / Processes")
{
$where1.="AND ( faction.document_field_id=125 AND  (faction.value='".$action."' or faction.value='NEW PROCESSES / EQUIPMENT ADOPTION') AND b.`id` = faction.`document_id` )";
}else if ($action=="Projects / Loans")
{
$where1.="AND ( faction.document_field_id=122 AND (faction.value='".$action."' or faction.value='PROJECTS / LOANS' )  AND b.`id` = faction.`document_id` )";
}
else if ($action=="Organization of Forums and Events")
{
$where1.="AND ( faction.document_field_id=122 AND ( faction.value='Organization of Forums and Events' OR faction.value='ORGANIZATION OF FORUMS AND EVENTS' ) AND b.`id` = faction.`document_id` )";
}
else if ($action=="Attendance to Forums and Events")
{
$where1.="AND ( faction.document_field_id=122 AND (faction.value='".$action."' or faction.value='ATTENDANCE TO FORUMS AND EVENTS' ) AND b.`id` = faction.`document_id` )";
}
else if ($action=="Awards delivered")
{
$where1.="AND ( faction.document_field_id=122 AND (faction.value='".$action."' or faction.value='AWARDS DELIVERED' ) AND b.`id` = faction.`document_id` )";
}
else if ($action=="Document Publishing")
{
$where1.="AND ( faction.document_field_id=122 AND (faction.value='".$action."' or faction.value='DOCUMENT PUBLISHING' )  AND b.`id` = faction.`document_id` )";
}
else if ($action=="Foundation/Cofoundation of organisms")
{
$where1.="AND ( faction.document_field_id=122 AND (faction.value='".$action."' or faction.value='FOUNDATION/COFOUNDATION OF ORGS' )  AND b.`id` = faction.`document_id` )";
}else if ($action=="Cost Reduction")
{
$where1.="AND ( faction.document_field_id=184 AND (faction.value='".$action."' or faction.value='COST REDUDCTION' ) AND b.`id` = faction.`document_id` )";
}else if ($action=="Capital Finance Raise")
{
$where1.="AND ( faction.document_field_id=184 AND (faction.value='".$action."' or faction.value='CAPITAL FINANCE RISE' )  AND b.`id` = faction.`document_id` )";
}else if ($action=="Increase Prices")
{
$where1.="AND ( faction.document_field_id=184 AND (faction.value='".$action."' or faction.value='INCREASE PRICES' )  AND b.`id` = faction.`document_id` )";
}else if ($action=="Volume Raise")
{
$where1.="AND ( faction.document_field_id=184 AND (faction.value='".$action."' or faction.value='VOLUME RAISE' )  AND b.`id` = faction.`document_id` )";
}
}
...
$from1.=",`document_fields_link` as ftype ";
 ...
 if ($type=="Cement")
{
$where1.="AND ( ftype.document_field_id=116 AND ( ftype.value='".$type."' or ftype.value='CEMENT') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Concrete")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='Concrete')  AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Aggregates")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='AGGREGATES') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Other")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='OTHER BUILDING MATERIAL')  AND b.`id` = ftype.`document_id` )";
}

else if ($type=="Quarries")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='QUARRIES') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Sea Terminal")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='SEA TERMINAL') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Distribution Center")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='DISTRIBUTION CENTER') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="RandB Center")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='R&D CENTER' or ftype.value='R&D') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Non Core Related")
{
$where1.="AND ( ftype.document_field_id=116 AND  ( ftype.value='".$type."' or ftype.value='NON-CORE RELATED')  AND b.`id` = ftype.`document_id` )";
}
else if ($type=="TDF(Tires)")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='AF. TDF(TIRES)') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Bio-Mass")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='AF BIO-MASS') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Industrial Wastes")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='Af. INDUSTRIAL WASTES') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Municipal Waste")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='AF: MUNICIPAL WASTES') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Low resource consumption")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='NP. LOW RESOURCE CONSUMPTION') AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Recyclers")
{
$where1.="AND ( ftype.document_field_id=126 AND  ( ftype.value='".$type."' or ftype.value='NP. RECYCLERS') AND b.`id` = ftype.`document_id` )";
}
...
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Business Incubator' or ftype.value='BUSINES INCUBATOR' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="CHILDREN")
{
$where1.="AND ( ftype.document_field_id=123 AND ftype.value='".$type."' AND b.`id` = ftype.`document_id` )";
}
else if ($type=="CONSERVING RESOURCES AND RESOURCE RECOVERY")
{
$where1.="AND ( ftype.document_field_id=123 AND ftype.value='".$type."' AND b.`id` = ftype.`document_id` )";
}
else if ($type=="HEALTH")
...
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Redeployment on closure' or ftype.value='REDEPLOYMENT ON CLOSURE' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Infraestructure Development")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Infraestucture Development' or ftype.value='INFRAESTRUCTURE DEVELOPMENT' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Housing")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Housing' or ftype.value='HOUSING' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Natural Disaster Aid")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Natural Disaster Aid' or ftype.value='NATURAL DISASTERS AID' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Historical Monuments Protector")
{
$where1.="AND ( ftype.document_field_id=123 AND ( ftype.document_field_id=123 AND (ftype.value='Historical Monuments Protector' or ftype.value='HISTORICAL MONUMENTS PROTECTION' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Safety promotion")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Safety promotion' or ftype.value='SAFETY PROMOTION' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Labor Equity/Human Rights")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Labor Equity/Human Rights' or ftype.value='LABOR EQUITY / HUMANS RIGHTS' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Sponsor of Cultural-Sport Activities")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Sponsor of Cultural-Sport Activities' or ftype.value='SPONSOR OF CULTURAL - SPORT ACTIVITIES' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Stakeholders/Community Involvement")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Stakeholders/Community Involved' or ftype.value='SKATEHOLDERS / COMMUNITY INVOLVEMENT AND PARTNERSHIPS' ) AND b.`id` = ftype.`document_id` )";
}
...
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Protecting Air Quality and Mitigating Disturbances' or ftype.value='PROTECTING AIR QUALITY AND MITIGATING DISTURBANCE' ) AND b.`id` = ftype.`document_id` )";
}
else if ($type=="Water Protection")
{
$where1.="AND ( ftype.document_field_id=123 AND (ftype.value='Water Protection' or ftype.value='WATER PROTECTION' ) AND b.`id` = ftype.`document_id` )";
}
...
if ($competitive=="Plan")
{
$where1.="AND ( fcompetitive.document_field_id=117 AND (fcompetitive.value='".$competitive."' or fcompetitive.value='PLAN' ) AND b.`id` = fcompetitive.`document_id` )";
}
else if ($competitive=="In Process")
{
$where1.="AND ( fcompetitive.document_field_id=117 AND  (fcompetitive.value='".$competitive."' or fcompetitive.value='IN PROCESS' ) AND b.`id` = fcompetitive.`document_id` )";
}
else if ($competitive=="Completed")
{
$where1.="AND ( fcompetitive.document_field_id=117 AND  (fcompetitive.value='".$competitive."' or fcompetitive.value='COMPLETED' ) AND b.`id` = fcompetitive.`document_id` )";
}
}
//======================
if($sustainability=="")
...
{
$where1.="AND ( fsustainability.document_field_id=124 AND (fsustainability.value='".$sustainability."' or  fsustainability.value='ECONOMIC') AND b.`id` = fsustainability.`document_id` )";
}
else if ($sustainability=="Social")
{
$where1.="AND ( fsustainability.document_field_id=124 AND (fsustainability.value='".$sustainability."' or  fsustainability.value='SOCIAL') AND b.`id` = fsustainability.`document_id` )";
}
else if ($sustainability=="Enviroment")
{
$where1.="AND ( fsustainability.document_field_id=124 AND (fsustainability.value='".$sustainability."' or  fsustainability.value='ENVIRONMENT') AND b.`id` = fsustainability.`document_id` )";
}
else if ($sustainability=="Sustainable")
{
$where1.="AND ( fsustainability.document_field_id=124 AND (fsustainability.value='".$sustainability."' or  fsustainability.value='SUSTAINABLE') AND b.`id` = fsustainability.`document_id` )";
}
}
//================
...
{
$where1.="AND ( freactiontype.document_field_id=118 AND (freactiontype.value='".$reactiontype."' or freactiontype.value='POSITIVE' ) AND b.`id` = freactiontype.`document_id` )";
}
else if ($reactiontype=="Negative")
{
$where1.="AND ( freactiontype.document_field_id=118 AND (freactiontype.value='".$reactiontype."' or freactiontype.value='NEGATIVE' ) AND b.`id` = freactiontype.`document_id` )";
}
...
else if ($consequence=="PROTEST / DEMONSTRATIONS")
{
$where1.="AND ( fconsequence.document_field_id=120 AND fconsequence.value='".$consequence."' AND b.`id` = fconsequence.`document_id` )";
}
...
$from1.=",`document_fields_link` as factor ";
 ...
 if ($actor=="Community")
{
$where1.="AND ( factor.document_field_id=121 AND (factor.value='".$actor."' or factor.value='COMMUNITY') AND b.`id` = factor.`document_id` )";
}
else if ($actor=="NGO/Media")
{
$where1.="AND ( factor.document_field_id=121 AND (factor.value='".$actor."' or factor.value='NGO / MEDIA') AND b.`id` = factor.`document_id` )";
}
else if ($actor=="Government")
{
$where1.="AND ( factor.document_field_id=121 AND (factor.value='".$actor."' or factor.value='GOVERNMENT') AND b.`id` = factor.`document_id` )";
}
else if ($actor=="Other Competitors")
{
$where1.="AND ( factor.document_field_id=121 AND (factor.value='".$actor."' or factor.value='OTHER COMPETITORS') AND b.`id` = factor.`document_id` )";
}
...
if ($letra) echo "&letra=".$letra;
if ($nota) echo "&nota=".$nota;
if ($industria) echo "&industry=".$industria;
if ($region) echo "&region=".$region;
if ($event) echo "&event=".$event;
if ($action) echo "&action=".$action;
if ($type) echo "&type=".$type;
...
if ($actor) echo "&actor=".$actor;
if ($competidor) echo "&competidor=".$competidor;
if ($subindustria) echo "&subindustry=".$subindustria;
if ($parampais) echo "&pais=".$parampais;
if ($ano) echo "&ano=".$ano;
if ($fechaA) echo "&fecha1=".$fechaA;
if ($fechaP) echo "&fecha2=".$fechaP;
if ($apartir) echo "&partir=".$apartir;
...
$prefrom ="SELECT distinct b.`id` FROM (select id,modified from `documents` where `document_type_id`=38  AND `status_id`<=2) as b from1  WHERE 1 where1 ";

}
else
{
$busq_links="SELECT distinct funion.id FROM ( ";
$busq_links.="(select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b ".$from0.$from1." WHERE 1 ".$where1.$where0." )";
$busq_links.= " UNION ( select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b ".$from2.$from1." WHERE 1 ".$where1.$where2." )" ;
$busq_links.= " UNION ( select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b ".$from3.$from1." WHERE 1 ".$where1.$where3." )) as funion  where 1".$limite;

$prefrom= "SELECT distinct funion.id FROM ( ";
$prefrom.="(select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b from0 from1  WHERE 1 where1 where0 )";
$prefrom.= " UNION ( select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b from2 from1  WHERE 1  where1 where2  )" ;
$prefrom.= " UNION ( select distinct b.id from (select id,modified from `documents` where `document_type_id`=38  AND `status_id`=1) as b from3 from1 WHERE 1 where1 where3  )) as funion  where 1" ;
...
//echo $busq_links;
$_SESSION["elquer"]=$busq_links;
echo "<tr><td><a href='/FichasBD/branches/SY/control.php?action=getwordsummwidg'>..</a></td></tr>";
echo "<tr>
...
echo "<td><a href='/FichasBD/branches/SY/control.php?action=getfeventb2b&letra=".$busq_links2[0]."' target='_blank' ><font size=1 face='Times'>".$pretit."  &nbsp</font></a></td>";
}
...
if ($typeS!=""){
 echo "<a href='/FichasBD/branches/SY/control.php?action=getfeventb2b&letra=".$busq_links2[0]."' target='_blank' ><font size=1 face='Times'>".$col5susta.$typeS."  &nbsp</font></a>";
