...
$busq_links="SELECT a.`document_id` FROM `document_fields_link`as a,`documents`as b  WHERE (a.`document_field_id`=92 OR a.`document_field_id`=71)  AND b.`id`= a.`document_id`AND b.`status_id`<=2 AND b.`document_type_id`=22 AND a.`value` LIKE '".$letrasar[0]."%'";
...
if ($indv_rel2[2]==18 ){
 ...
 echo "<a href='/FichasBD/branches/SY/control.php?action=getfinst&letra=".$busq_links2[0]."' target='fichas2' ><font size=2 face='ARIAL'>".$nomcom." ".$raz." ".$fechfun  ."</font></a>";
}
//}
