...
<tr>
    <td><table width=20% height=30%  align='left' style='text-align: left' ; ' border='0'  cellpadding='2' cellspacing='2'><font size=1 face='Arial'><tbody>";

	$insmod=0;
   for($i=0; $i<count($letrasar)-1;$i++){
	$busq_links="SELECT a.`document_id`,b.`modified`  FROM ( select * from `document_fields_link` where `document_field_id`=21 AND `value` LIKE '".$letrasar[$i]."%') as a,`documents` as b  WHERE b.`id`= a.`document_id` AND b.`status_id`<=2 ORDER BY b.`modified` DESC ";
	...
	$oDocument = & Document::get($busq_links2[0]);
 	 ...
 	 echo "<tr><td></td><td><table><tr><td>";



	 echo displayButton2("_blank","reporte", "idd=".$busq_links2[0], "Reporte");
	echo "</td><td>";

	echo "<a href='/FichasBD/branches/SY/control.php?action=getword&letra=".$busq_links2[0]."' target='_blank' ><font size=2 face='Times'>&nbsp;&nbsp;&nbsp;w</font></a>";
	...
	echo "<tr style='background-color: rgb($red, $green,$blue);'><td valign='top' height=40% align='center'>";
 	...
 	echo  "<img style=".'"'." width: 60px; height: 60px;".'"'." alt= ".'"'.$oDocument->getName().'"'." src= ".'"'."http://proveedores.intelect.com.mx/FichasBD/branches/SY/Documents". $oDocument->generateFullFolderPath($oDocument->getFolderID())."/".$oDocument->getFileName().'"'." > </a></td>
...
<td >";
	...
	$genero0= "SELECT value FROM `document_fields_link` WHERE `document_field_id`=69  AND `document_id`=".$busq_links2[0];
	$genero1 = mysql_query($genero0);
	$genero2 = mysql_fetch_row($genero1);

	if ($genero2[0]=="Femenino")
	{
	 echo "<img style=".'"'." width: 60px; height: 60px;".'"'." alt= ".'"'.$oDocument->getName().'"'." src=\"http://proveedores.intelect.com.mx/FichasBD/branches/SY/presentation/lookAndFeel/knowledgeTree/documentmanagement/nofotof.jpg\" ></td>
<td >";
	}
	else
	{
	 echo "<img style=".'"'." width: 60px; height: 60px;".'"'." alt= ".'"'.$oDocument->getName().'"'." src=\"$default->rootUrl/Documents/Root Folder/Default Unit/SIDEM/Individuo/nofoto.jpg\" ></td>
<td >";
	}
...
if ($indv_rel2[2]==65 ){
 ...
 echo "<a href='/FichasBD/branches/SY/control.php?action=getficha&letra=".$busq_links2[0]."' target='fichas2' ><font size=1 face='Arial'>".$appat." ".$apmat." ".$nam  ." </font></a>
