...
$busq_links="SELECT a.`document_id` FROM `document_fields_link`as a,`documents`as b  WHERE a.`document_field_id`=78 AND b.`id`= a.`document_id`AND b.`status_id`<=2 AND b.`document_type_id`= 2  AND  a.`value` like '". $valor."%'";
...
//=========>>>>> FUNCION PARA LIMITAR EL TAMAÑO DE TEXTO ========>>>
...
//============<<<<< TERMINA FUNCION  <<<<<===================
 ...
 echo "<a href='/FichasBD/branches/SY/control.php?action=getfevento&letra=".$busq_links2[0]."' target='fichas2' ><font size=2 face='Arial' color='blue'>".$fevent.".-</font><font size=2 face='Arial'> ".$texto." ".$tip  ." ".$indv_rel2[1]."</font></a>
