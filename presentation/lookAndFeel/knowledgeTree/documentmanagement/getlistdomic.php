...
$busq_links="SELECT a.`document_id`,c.`value` FROM `document_fields_link`as a,`documents`as b,`document_fields_link`as c  WHERE a.`document_field_id`=18 AND b.`id`= a.`document_id`AND b.`status_id`<=2 AND b.`document_type_id`= 9  AND c.`value` LIKE '".$letrasar[$i]."%' AND c.`document_id`=a.`value`";
...
if ($indv_rel2[2]==135 ){
 ...
 echo "<a href='/FichasBD/branches/SY/control.php?action=getfDomic&letra=".$busq_links2[0]."' target='fichas2' ><font size=2 face='Times'>".$Dir."  ".$call." ".$nume."  ".$col."  ".$Lug[1] ."</font></a>
